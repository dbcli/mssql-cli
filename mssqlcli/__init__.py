...
from mssqlcli import mssqlutils
...
MetaQuery = namedtuple(
    ...
    [
        ...
        'contains_password',    # True if any subquery contains the PASSWORD keyword
    ])
...
class MssqlCli(object):
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # statement.
                if mssqlutils.quit_command(document.text):
                    raise EOFError
                ...
                if not query.contains_password:
                    with self._completer_lock:
    ...
    def _evaluate_command(self, text):
        ...
        for rows, columns, status, sql, is_error in \
            ...
            contains_password = mssqlutils.has_password_in_cmd(sql)
            ...
            formatted = mssqlutils.format_output(None, rows, columns, status, settings)
            output.extend(formatted)

            db_changed, new_db_name = mssqlutils.has_change_db_cmd(sql)
        ...
        return output, MetaQuery(
            text, all_success, total, meta_changed, db_changed, path_changed, mutated, contains_password)
